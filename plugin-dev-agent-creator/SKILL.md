...
description: Generates Claude Code plugin agent files from a user request. Use when the user asks to create an agent, generate an agent, build a new agent, make an agent, or add an autonomous agent to a plugin.
...
Original Claude Code metadata: model `sonnet`, color `magenta`, tools `Write`, `Read`.
