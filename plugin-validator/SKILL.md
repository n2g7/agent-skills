...
description: Validates Claude Code plugin structure, plugin.json, commands, agents, skills, hooks, and MCP config. Use when the user asks to validate a plugin, check plugin structure, verify plugin.json, check plugin files, or after creating or modifying plugin components.
...
Original Claude Code metadata: model `inherit`, color `yellow`, tools `Read`, `Grep`, `Glob`, `Bash`.
