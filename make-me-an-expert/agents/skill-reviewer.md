...
3. Read `SKILL.md`. Then read supporting files that `SKILL.md` points to (`templates/`, `references/`, `scripts/`, `agents/`) as needed. Do not bulk-read the rest of the skills library.
...
## Extra check: STE and unslop

Confirm `SKILL.md` requires `ste-writing` and `unslop` on generated pack Markdown and teaching replies. Confirm this pass runs before PDF render. Confirm those two skills do not count toward the compose cap of 4.
