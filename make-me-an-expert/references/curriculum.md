...
Write the book in STE. Then run `unslop` on it (`ste-pass.md`). Then render the PDF.

## Expertise ceiling

Aim for a competent practitioner with a map of the frontier. Say so in the book. The pack does not certify expertise. It mustn't claim the user is now an expert.
...
2. **Diagnostic**: "you said you know X" as a 1-page check. Probes from intake. Not a recap of the self-rating. Mark what the pack will still teach.
3. **Foundations, then core, then advanced, then frontier**: one chapter per idea cluster. Match depth to intake goal and ceiling. Do not skip foundations because they claimed knowledge.
...
Resolve `<skills-root>` as: project `.agents/skills`, else this repo root, else `~/.agents/skills/`. Markdown to HTML to PDF. WeasyPrint first. Pandoc as fallback.
...
After gap-fill, run STE and unslop again. Then re-render this PDF (or a short supplement PDF under `supplements/`).

## Safety line

Put this in the coursebook and the PDF front matter:

> Educational material only. Not professional advice. Especially not medical, legal, or financial advice. Check primary sources and qualified practitioners before you act.

## Honest limits

This agent cannot watch YouTube or ingest whole books. Point at `watchlist.md` and citations. Distilled notes in `kb/` are the teaching source. The PDF is the human-readable coursebook. It is not a copyrighted dump.
